...
def add_background(img, img2):
    ...
    for c in range(0, 3):
        img2[y1:y2, x1:x2, c] = (alpha_s * img[:, :, c] + alpha_l * img2[y1:y2, x1:x2, c])
...
def add_photo(img , photo):
    ...
    for c in range(0, 3):
        img[y1:y2, x1:x2, c] = (alpha_s * photo[:, :, c] + alpha_l * img[y1:y2, x1:x2, c])
...
def id_card(image_path , background_path , photo_path):
    ...
    cv2.putText(img, text=college() , org = (details["college"][0] , details["college"][1]), fontFace= font1, 
                fontScale= details["font_size"] ,color = (0,0,0,255), thickness=details['thickness'], lineType=cv2.LINE_AA)
    cv2.putText(img, text=address() , org = (details["col_address"][0] , details["col_address"][1])  , fontFace= font1, 
                fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

    if(choice_for_validity()):
        cv2.putText(img, text="Validity : " + valitdity() , org = (details["validity"][0] , details["validity"][1])  , fontFace= font2, 
                fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

    if(choice_for_name_dob_branch_address_key()):

        cv2.putText(img, text="NAME : " + name() , org = (details["name"][0] , details["name"][1])  , fontFace= font2, 
                fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

        cv2.putText(img, text="BRANCH : " + branch() , org = (details["branch"][0] , details["branch"][1])  , fontFace= font2, 
                fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

        if(choice_for_dob()):
            cv2.putText(img, text="DoB : " + date_of_birth()  , org = (details["dob"][0] , details["dob"][1])  , fontFace= font2, 
                fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

            if(choice_for_address()):
                cv2.putText(img, text="ADDRESS : " + address() , org = (details["stu_address"][0] , details["stu_address"][1] + 50)  , fontFace= font2, 
                    fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

        elif(choice_for_address()):
            cv2.putText(img, text="ADDRESS : " + address() , org = (details["stu_address"][0] , details["stu_address"][1])  , fontFace= font2, 
                fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

    else:
            
        cv2.putText(img, text=name() , org = (details["name"][0] , details["name"][1])  , fontFace= font2, 
                    fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

        cv2.putText(img, text=branch() , org = (details["branch"][0] , details["branch"][1]), fontFace= font2, 
                    fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)
        ...
        if(choice_for_dob()):
            ...
            if(choice_for_address()):
                cv2.putText(img, text=address() , org = (details["stu_address"][0] , details["stu_address"][1] +50) , fontFace= font2, 
                            fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)

        elif(choice_for_address()):
            cv2.putText(img, text=address() , org = (details["stu_address"][0] , details["stu_address"][1])  , fontFace= font2, 
                        fontScale=1,color = (0,0,0,255),thickness=2, lineType=cv2.LINE_AA)
